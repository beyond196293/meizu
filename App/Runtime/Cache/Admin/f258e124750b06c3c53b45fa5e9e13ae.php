    ...
    <form method="post" class="form-x" action="" enctype="multipart/form-data">    <!--当方法名与文件名一直的时候action里面可以不加U地址;-->
     ...
     <input type="hidden" class="input w50" value="<?php echo ($i); ?>" name="i" readonly="readonly" />
      <div class="form-group">
        ...
        <div class="field">
          <img src="/obj/Public/Uploads/<?php echo ($list["image"]); ?>" alt="" width="120" height="50" />
      ...
      <div class="form-group">
        ...
        <div class="field">
          ...
          <?php if(is_array($lis)): foreach($lis as $key=>$val): ?><option value="<?php echo ($val["id"]); ?>" selected><?php echo ($val["id"]); ?></option><?php endforeach; endif; ?>
          </select>
        </div>
      </div>
        <div class="field">
