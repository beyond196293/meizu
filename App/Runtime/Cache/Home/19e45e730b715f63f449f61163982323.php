        ...
        <div class="mzcontainer">
            ...
            <div class="mzcontainer order-product">
				...
				<table cellpadding="0" cellspacing="0">
					...
					<?php if(is_array($list)): foreach($list as $key=>$asd): ?><tbody>
						<tr>
							<td class="order-product-table-name">
								<img class="order-product-image" src="/obj/Public/Uploads/<?php echo ($asd["iname"]); ?>">
								<div class="order-product-name">
									<a target="_blank" class="order-product-link" id="name" href="http://detail.meizu.com/item/meilan_e1.html"><?php echo ($asd["gname"]); ?><br><?php echo ($asd["net"]); ?>&nbsp;<?php echo ($asd["colors"]); ?>&nbsp;<?php echo ($asd["memory"]); ?></a>
								</div>
							</td>
							<td class="order-product-table-price"><p></p><span class="order-product-price"><?php echo ($asd["price"]); ?></span></td>
							<td class="order-product-table-num" id="num"><?php echo ($asd["nnum"]); ?></td>
							<td class="order-product-table-total"><p class="order-product-price red"><?php echo ($asd["tnum"]); ?></p></td>
					...
					<tfoot>
						<tr>
							<td colspan="5">
								...
								<div class="order-product-total">合计：<span class="order-product-price red"><?php echo ($mar); ?></span></div>
							</td>
						</tr>
					</tfoot>
				</table>
            ...
            <div class="order-total clearfix">
                <div class="order-total-content">
                    <div class="order-total-row">
                        总金额
                        <div class="order-total-price" id="totalOrderPrice"><?php echo ($mar); ?></div>
                    </div>
                    ...
                    <div class="order-total-row">
                        应付：
                        <div class="order-total-price total" id="totalPayPrice"><?php echo ($mar); ?></div>
                    </div>
                    <div class="order-total-row order-total-valid-row clearfix">
                        <a class="mz-btn success" id="submitForm" href="<?php echo U('Home/Confimorder/car',array('total'=>$mar));?>">提交订单</a>
                    </div>
	...
	function buyback(){
		...
		$.get('<?php echo U("Home/Back/sel");?>',{'code':code},function(result){
			...
			if(a[0]){
				...
				$('#submitForm').attr('href',"<?php echo U('Home/Confimorder/car');?>?buyback="+a[1]+"&total="+money);
			}else{
				alert('您的回购劵号不正确');
			}
		});
	...
	//alert($('#discount option:selected').val());
			if($('#discount option:selected').val()!=""){
				...
				var money=$('#totalOrderPrice').html()-$('#discount option:selected').html();
					...
					$('#totalPayPrice').html(money);
				$('#submitForm').attr('href',"<?php echo U('Home/Confimorder/car');?>?discount="+$('#discount option:selected').attr('aa')+"&total="+money);
			}else{
