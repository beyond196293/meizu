        ...
        <div class="mzcontainer">
            ...
            <div class="order-address" id="addressModule">
                ...
                <ul class="order-address-list clearfix" id="addressList" style="height: 174px;">
					<?php if(!empty($info)): ?><li class="order-address-checkbox checked">
						<div class="order-address-checkbox-top">
							<div class="order-address-checkbox-name"><?php echo ($info["name"]); ?></div>
							<div class="order-address-checkbox-phone"><?php echo ($info["tel"]); ?></div>
						</div>
						<div class="order-address-checkbox-content"><?php echo ($info["address"]); ?></div>
					...
					<li class="order-address-checkbox add">
						...
						<a href="<?php echo U('Home/Address/index');?>"><div class="order-address-add-text">添加新地址</div></a>
					</li>
				</ul>
					...
					<tbody>
						<tr>
							<td class="order-product-table-name">
								<img class="order-product-image" src="/obj/Public/Uploads/<?php echo ($a["iname"]); ?>">
								<div class="order-product-name">
									<a target="_blank" class="order-product-link" id="name" href="http://detail.meizu.com/item/meilan_e1.html"><?php echo ($a["gname"]); ?><br><?php echo ($a["net"]); ?>&nbsp;<?php echo ($a["color"]); ?>&nbsp;<?php echo ($a["memory"]); ?></a>
								</div>
							</td>
							<td class="order-product-table-price"><p></p><span class="order-product-price"><?php echo ($a["price"]); ?></span></td>
							<td class="order-product-table-num" id="num"><?php echo ($a["num"]); ?></td>
							<td class="order-product-table-total"><p class="order-product-price red"><?php echo ($a["price"]); ?></p></td>
					...
					<tfoot>
						<tr>
							<td colspan="5">
								...
								<div class="order-product-total">合计：<span class="order-product-price red"><?php echo ($a["price"]); ?></span></div>
							</td>
						</tr>
					</tfoot>
				</table>
            ...
            <div class="order-total clearfix">
                <div class="order-total-content">
                    <div class="order-total-row">
                        总金额
                        <div class="order-total-price" id="totalOrderPrice"><?php echo ($a["price"]); ?></div>
                    </div>
                    ...
                    <div class="order-total-row">
                        应付：
                        <div class="order-total-price total" id="totalPayPrice"><?php echo ($a["price"]); ?></div>
                    </div>
                    <div class="order-total-row order-total-valid-row clearfix">
                        <a class="mz-btn success" id="submitForm" href="<?php echo U('Home/Confimorder/affirm',array('total'=>$a['price'],'num'=>$a['num'],'gname'=>$a['gname'],'memory'=>$a['memory'],'colors'=>$a['colors'],'net'=>$a['net']));?>">提交订单</a>
                    </div>
...
<script>
	...
	function buyback(){
		code=$('#text').val();
		$.get('<?php echo U("Home/Back/sel");?>',{'code':code},function(result){
			//alert(result);
			a=result.split(",");
			if(a[0]){
				$('#nu').html('1');
				...
				$('#submitForm').attr('href',"<?php echo U('Home/Confimorder/affirm');?>?buyback="+a[1]+"&num=<?php echo ($a["num"]); ?>&gname=<?php echo ($a["gname"]); ?>&memory=<?php echo ($a["memory"]); ?>&colors=<?php echo ($a["colors"]); ?>&net=<?php echo ($a["net"]); ?>&total="+money);
			}else{
				alert('您的回购劵号不正确');
			}
		});
	...
	//alert($('#discount option:selected').val());
			if($('#discount option:selected').val()!=""){
				...
				var money=$('#totalOrderPrice').html()-$('#discount option:selected').html();
					...
					$('#totalPayPrice').html(money);
				$('#submitForm').attr('href',"<?php echo U('Home/Confimorder/affirm');?>?discount="+$('#discount option:selected').attr('aa')+"&num=<?php echo ($a["num"]); ?>&gname=<?php echo ($a["gname"]); ?>&memory=<?php echo ($a["memory"]); ?>&colors=<?php echo ($a["colors"]); ?>&net=<?php echo ($a["net"]); ?>&total="+money);
			}else{
